    ...
    def __init__(self, training_pipeline_config: TrainingPipelineConfig):
        try:
            self.data_ingestion_dir = os.path.join(
                ...
                common_constants.DATA_INGESTION_DIR_NAME
            )
            self.data_ingestion_ingested_dir = os.path.join(
                self.data_ingestion_dir, common_constants.DATA_INGESTION_INGESTED_DIR
            )
            self.data_ingestion_ingested_flights_file_path = os.path.join(
                self.data_ingestion_ingested_dir,
                common_constants.DATA_INGESTION_INGESTED_FLIGHTS_FILE_NAME
            )
            self.data_ingestion_ingested_airports_file_path = os.path.join(
                self.data_ingestion_ingested_dir,
                common_constants.DATA_INGESTION_INGESTED_AIRPORTS_FILE_NAME
            )
            self.data_ingestion_ingested_airlines_file_path = os.path.join(
                self.data_ingestion_ingested_dir,
                common_constants.DATA_INGESTION_INGESTED_AIRLINES_FILE_NAME
            )
            self.data_ingestion_ingested_holidays_file_path = os.path.join(
                self.data_ingestion_ingested_dir,
                common_constants.DATA_INGESTION_INGESTED_HOLIDAYS_FILE_NAME
            )
            self.data_ingestion_ingested_weather_file_path = os.path.join(
                self.data_ingestion_ingested_dir,
                common_constants.DATA_INGESTION_INGESTED_WEATHER_FILE_NAME
            )
        except Exception as e:
            raise CustomException(e, sys)
